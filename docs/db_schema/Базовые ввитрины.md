...
fact_order_position }o--|| dim_currency : ""
...
dim_client {
    client_sk bigserial PK "суррогатный ключ клиента"
    ...
    version int "Версия загрузки"
    is_current boolean "Флаг актуальности"
}
```
...
dim_product {
    product_sk bigserial PK "Суррогатный ключ продукта"
...
dim_pick_up_point {
    point_sk bigserial PK "Суррогатный ключ точки выдачи"
...
dim_currency {
    currency_sk bigserial PK "Суррогатный ключ валюты"
...
fact_order_position {
    order_position_sk bigserial PK "Суррогатный ключ позиции по заказу"
    order_id text "ID заказа"
    client_sk bigint FK "Ссылка на клиента"
    client_id text "ID клиента"
    pick_up_point_sk bigint FK "Ссылка на пункт выдачи"
    pick_up_point_id text "Id пункта выдачи"
    product_sk bigint FK "Ссылка на продукт"
    ...
    total_price_currency_sk bigint FK "Ссылка на валюту стоимости заказа"
    total_price_currency_code text "Код валюты заказа"
...
fact_supply_position {
    supply_position_sk bigserial PK "Суррогатный ключ позиции поставки"
    ...
    version_id int "Версия загрузки"
}
```
...
fact_shipment_position {
    shipment_position_sk bigserial PK "Суррогатный ключ позиции отгрузки"
