...
raw_client {
    ...
    load_dttm timestamptz PK "Время загрузки заипси с источника"
}
...
raw_product {
    ...
    price_currency text "Буквенный код валюты цены"
    src_system text "Наименование системы-источника"
    load_dttm timestamptz PK "Время загрузки заипси с источника"
}
...
raw_supply_item {
    supply_id text PK,FK "ID поставки"
    product_id text PK,FK "ID продукта"
...
raw_shipment_item {
    shipment_id text PK,FK "ID отгрузки"
    product_id text PK,FK "ID продукта"
...
raw_order {
    ...
    client_id text FK "ID клиента"
    pick_up_point_id text FK "ID пункта выдачи"
    status text "Статус заказа"
    track_number text "Трек-номер заказа"
    total_price_value int "Итогоавая цена"
    total_price_currency text FK "ID валюты"
...
raw_pick_up_point {
    ...
    address_country text "Страна нахождения"
    address_region text "Регион"
    address_city text "Город" 
    address_street text "Название улицы"
    address_house text "Номер дома"
